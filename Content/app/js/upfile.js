            ...
            if (!obj.hasClass("setUpFiled")) {
                ...
                obj.ajaxUploader({
                    ...
                    onSuccess: function (e, file, t, data) {
                        ...
                        if (jQuery(dataObj.target).hasClass("attach-template"))
                        {
                            var template = jQuery("#change_attachments_template").html();
                            template = template.replace("#src", data.FilePath);
                            template = template.replace("#srcs", data.FilePath);
                            template = template.replace("#filetitle", data.FileName);
                            template = template.replace("#filename", dataObj.fileName);
                            template = template.replace("#filenamevalue", data.FileName);
                            template = template.replace("#filenamedown", item.FileName);
                            template = template.replace("#filepath", dataObj.filePath);
                            template = template.replace("#filepathvalue", data.FilePath);
                            template = template.replace("#filepathdown", data.FilePath);
                            template = template.replace("#filetarget", dataObj.target);
                            template = template.replace("#filenames", dataObj.fileName);
                            template = template.replace("#filepaths", dataObj.filePath);
                            jQuery("#R" + file.id).replaceWith(jQuery(template));
                            jQuery(dataObj.target).removeClass("loading");
                        }
                        else {
            ...
            if (!obj.hasClass("setUpFiled")) {
                ...
                obj.ajaxUploader({
                    ...
                    onSuccess: function (e, file, t, data) {
                        ...
                        if (targetObj.hasClass("ui-group-item")) {
                        ...
                        else if (dataObj.isonly == '1') {
                            //kiiểm tra xem nếu chỉ là ảnh thì hiện ảnh, k phải ảnh thì hiện link file đính kèm
                            if (data.FileName.match(/\.(jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF)$/))
                            {
                                targetObj.removeClass("loading").html(
                                    "<span class='fileitem member isNew'>" +
                                    "</br>" +
                                    "<img class='img-thumbnail' src='" +
                                    Cdata.SrcPath(data.FilePath) +
                                    "' title='" +
                                    data.FileName +
                                    "' />" +
                                    "<br/>" +
                                    "<input name='" +
                                    dataObj.fileName +
                                    "' class='fileNames' type='hidden' value='" +
                                    data.FileName +
                                    "'  />" +
                                    "<input name='" +
                                    dataObj.filePath +
                                    "' class='filePaths' type='hidden' value='" +
                                    data.FilePath +
                                    "'/>" +
                                    "<button type='button' class='btn btn-xs btn-link close delMember'>x</button>" +
                                    "</span>"
                                ...
                                targetObj.removeClass("loading").html(
                                    "<span class='fileitem member isNew'>" + "</br>" +
                                    "<a class='text-blue' target='_blank' href='" + Cdata.SrcPath(data.FilePath) + "' title='" + data.FileName + "'> " + data.FileName + "</a>" + " <br />" +
                            ...
                            if (data.FileName.match(/\.(jpg|jpeg|png|gif|JPG|JPEG|PNG|GIF)$/)) {
                                ...
                                targetObj.removeClass("loading").append(
                                    "<span class='fileitem member isNew'>" + "</br>" +
                                    "<img class='img-thumbnail' src='" + Cdata.SrcPath(data.FilePath) + "' title='" + data.FileName + "' />" + "<br/>" +
                                    "<input name='" + dataObj.fileName + "' class='fileNames' type='hidden' value='" + data.FileName + "'  />" +
                                    "<input name='" + dataObj.filePath + "' class='filePaths' type='hidden' value='" + data.FilePath + "'/>" +
                                    "<button type='button' class='btn btn-xs btn-link close delMember'>x</button>" +
                                    "</span>"
